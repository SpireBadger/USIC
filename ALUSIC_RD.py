...
try:
    ...
    ######--Main Text USIC---
    Dist=r"\\parcser02\GisServerManager\Data\AL.sde\GISADMIN.Gas\GISADMIN.MainText"
    # Location of existing GDB and name
    ...
    fieldNameList = []
    fields = arcpy.ListFields("Distsss")
    # Call out fields desired and delete others
    for field in fields:
        if field.name == 'OBJECTID':
            ...
            print 'keeping '+field.name
        elif field.name == 'FONTSIZE':
            print 'keeping '+field.name
        elif field.name == 'ANGLE':
            print 'keeping '+field.name
        elif field.name == 'FONTSIZE':
            print 'keeping '+field.name   
        else :
    ...
    if arcpy.Exists(USIC+"\\"+"MainTextUSIC.shp"):
        arcpy.Delete_management(USIC+"\\"+"MainTextUSIC.shp")
    arcpy.CopyFeatures_management ('Distsss', USIC+"\\"+"MainTextUSIC.shp")
...
# End code, delete below ish but leave Except clauses and del variables if present
    ...
    Dist=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_LAND.Landbase\LGC_LAND.MoNatDimLine"
    ...
    Dist=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.DistributionMainDimension"
    ...
    ######--Service Point--#########
    SP=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.ServicePoint"
    ...
    ################--Test Points
    TP=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.CPTestPoint"
    ...
    CF=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.ControllableFitting"
    ...
    NCF=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.NonControllableFitting"
    ...
    ##########--Abandoned Services
    AbandGasSrv=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.AbandonedGasService"
    ...
    ####------Drip-----######
    Drips=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.Drip"
    ...
    ########------Gas Valves-----######
    GasValves=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.GasValve"
    ...
    Dist=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.DistributionMain"
    ...
    arcpy.DeleteField_management("DistsssCalc","FIELDBOOKP")
    ############--Services#################
    Srv=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.Service"
    ...
    arcpy.DeleteField_management("DistsssCalc","FIELDBOOKP")
    ##############--Abandoned Gas Pipe
    AbandGasPipe=r"\\parcser02\GisServerManager\Data\AL.sde\LGC_GAS.GasFacilities\LGC_GAS.AbandonedGasPipe"
