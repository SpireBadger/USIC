...
##            oldAddr = row[1]
##            if mxLoc in svcDict:
##                print("Looking at row {0} with an address of {1}.".format(mxLoc, oldAddr))
##                row[1] = svcDict[mxLoc]
##                print("Address is now {0}.".format(row[1])
##                cursor.updateRow(row)
#    
#    with arcpy.da.UpdateCursor(testingSHP, ['SERVICEMXL','STREETADDR']) as cur:
#        for row in cur:
#            mxLoc = row[0]
#            oldAddr = row[1]
#            if mxLoc in svcDict:
